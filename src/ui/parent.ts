...
export function mountParent(root: HTMLElement, dict: Dictionary, current: Profile, act: ParentActions): void {
  ...
  function profileCard(p: Profile): HTMLElement {
    ...
    return h('div', { class: 'card' },
      ...
      bests.length ? h('div', { style: { marginTop: '8px' } }, h('strong', null, 'タイムアタック最高点: '), bests.map((b) => `${CATEGORY_LABEL_KANJI[b.cat]} ${b.level === 0 ? 'L1〜5通し' : `L${b.level} ${b.count}問`} ${b.score}点`).join(' ／ ')) : null,
  ...
  root.appendChild(h('div', { class: 'screen parent' },
    ...
    h('div', { class: 'card' },
      ...
      h('p', { class: 'sub' }, 'クリア画面の 🚩 を押すと、ここに設定した URL に送られ、スプレッドシートに記録されます。初期値は公開時に設定した URL です。別のシートにしたいときだけ書き換えてください。'),
      urlInput,
